...
STORM_BE_HOST="${STORM_BE_HOST:-backend.test.example}"

CDMI_ENDPOINT="${CDMI_ENDPOINT:-cdmi.test.example:8888}"
...
DAV_HOST=${DAV_HOST:-webdav.test.example}
GFTP_HOST=${GFTP_HOST:-gridftp.test.example}
GFTP_PORT=${GFTP_PORT:-2811}
SRM_HOST=${SRM_HOST:-frontend.test.example}
...
/assets/scripts/wait-for-it.sh ${SRM_HOST}:8444 --timeout=${WAIT_TIMEOUT}
